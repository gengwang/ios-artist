...
public class ViewController: UIViewController {
    ...
    static let yPanGap: CGFloat = 10000
    var translationY: CGFloat = 0 {
        didSet {
            let perc = CGFloat( Int64(translationY) % Int64(ViewController.yPanGap) ) / ViewController.yPanGap
            dLayer.srub(to: perc)
        }
    }
    class DigitLayer: CATextLayer {
        ...
        init(digit: String) {
            ...
            // Setting CATransform3D.m34 will enable the "3D perspective". Layers with negative values at the z axis will appear farther away from the user's eye
        ...
        func srub(to percentage: CGFloat) {
            print("srub to:    \(percentage)")
            backdropLayer.add(backdropPopInOutAnimation, forKey: nil)
            backdropPopInOutAnimation.timeOffset = backdropPopInOutAnimation.duration * CFTimeInterval(percentage)
            
            textLayer.add(textPopInOutAnimation, forKey: nil)
            textPopInOutAnimation.timeOffset = textPopInOutAnimation.duration * CFTimeInterval(percentage)
    ...
    override public func viewDidLoad() {
        ...
        // Set up the pan recognizer
        let recognizer = UIPanGestureRecognizer(target: self, action: #selector(ViewController.handlePan(recognizer:)))
...
//        let screenBounds = UIScreen.main.bounds
    ...
    @objc func handlePan(recognizer: UIPanGestureRecognizer) {
        if (recognizer.state == .ended) {
            ...
            // TODO: snap to the beginning and end of animation until the user lifts finger
            let velocityY: CGFloat = recognizer.velocity(in: self.view).y >= 0 ? 1.0 : -1.0
//            let velocityY: CGFloat = 1
            let translation = recognizer.translation(in: self.view)
            print("\ntranslation: \(translation.y)")
            print("volicity y: \(velocityY)")
            translationY += translation.y * velocityY
        }
    }
